...
def _default_steps(topic: str, weak: list[str], resources: list[dict]) -> list[dict]:
        ...
        steps.append(
            {
                ...
                "resource_ids": ids if not substeps else ids[:1],
                "estimated_minutes": minutes,
                "status": "pending",
                "substeps": substeps,
            }
        )
    return finalize_path_steps(steps)
    ...
    *,
    ...
    try:
        ...
        normalized = normalize_step_tree(
            steps, valid_ids=valid_ids, default_resource_ids=resource_ids
        )
        return finalize_path_steps(normalized) if normalized else None
    except Exception:
        return None
...
async def _llm_plan_by_topic(
    ...
    *,
    ...
    try:
        ...
        if not steps:
            return None
        normalized = normalize_step_tree(steps, valid_ids=set(), default_resource_ids=[])
        return finalize_path_steps(normalized) if normalized else None
    except Exception:
        return None
...
async def path_node(state: AgentState) -> dict:
    ...
    if not steps:
        steps = _default_steps(topic, weak, resources)
