...
REVIEWER_SYSTEM = (
    ...
    "评估给定资源节选是否能真正推动高校学生理解、练习或应用。\n"

    "输出一句话（不超过 30 字），格式：「通过：…」或「需注意：…」。\n"

    "检查：知识点是否明确，是否有具体例子/题目/代码/任务及详解，"
    "是否存在空洞套话、弱智题目、不可运行代码、装饰性图片或虚假来源。"
...
RESOURCE_GENERATION_BASE = (
    ...
    "你不是在生成普通学习资料，而是在生成可直接服务学习路径、AI 课堂、小测验和复习巩固的学习资源。\n"
    ...
    "只输出最终学习资源正文，统一使用干净 Markdown；Mermaid 仅放在 Markdown 代码块中。"
    "禁止输出 JSON、脏 JSON、字段转义文本、HTML 表单或生成过程说明。\n"

    "资源必须服务明确学习用途并围绕目标知识点展开；禁止只有大段百科讲解或空泛总结。\n"
    "每份正文必须用给定资源标题作为一级标题，并标明资源类型。开头必须有『资源导航』，"
    "明确对应知识点、学习用途、适合水平、前置知识和适用场景；"
    "随后必须有 80–120 字『资源摘要』、具体学习内容、常见误区、自检或验收任务、下一步建议和预期学习结果。\n"
    "具体内容必须包含该类型真正需要的例题、题目、代码、图、步骤或任务，并给出详细解析或操作说明。"
    "不允许无意义问题、没有详解的答案、不可运行或无中文注释的代码、装饰性多模态内容、虚构来源。\n"
    "画像适配：基础弱时用直观解释、最小例子和分步提示；基础好时增加应用、辨析和迁移；"
    "应试倾向时增加真实迷惑项与易错辨析；项目倾向时增加代码、交付物和扩展任务；"
    "困惑度或认知负荷高时减少长段落，改用短段、步骤和结构化卡片。\n"
    "若画像或实时状态给出薄弱点、错题或当前卡点，优先生成错因讲解、对比辨析和专项训练，"
    "不要平均分配篇幅。\n"
    "正文末尾增加『复用 metadata』小节，以 Markdown 列表写出 used_for（path/classroom/quiz/review）、"
    "难度、预期学习结果和可复用内容单元；不要用 JSON。\n"

    "数学公式必须使用 LaTeX：行内公式用 $...$（如 $\\frac{dy}{dx}$），"

    "独立一行公式用 $$...$$；禁止使用裸括号包裹（如 (\\int_0^1 x^2 dx)）。\n"

    "公式定界符必须成对并包住完整公式，LaTeX 命令不得出现在定界符外。"

    "例如梯度下降必须写成 $$\\theta_j := \\theta_j - \\alpha \\frac{\\partial}{\\partial \\theta_j} J(\\theta)$$，"

    "严禁写成 \\theta_j := ... J$\\theta$ 或只包住公式的一部分。输出前自行检查每个 $ 是否成对。"
...
RESOURCE_TYPE_INSTRUCTIONS: dict[str, str] = {
    ...
    "doc": (
        ...
        "生成『讲解文档』Markdown。必须包含知识点目标、前置知识、核心概念解释、至少 1 个典型例题、"
        "完整解题思路、编号解题步骤、最终答案、至少 1 个易错点、至少 1 个带答案和解析的自检问题、"
        "路径适用阶段、学习后检查和预期学习结果。例题必须能计算、判断、推导或完成任务，"
        "禁止用『请复述概念』充当例题；只生成 1 份时也不得降低密度。"
    ...
    "mindmap": (
        ...
        "必须分别有『核心概念、方法步骤、应用场景、易错点』分支，并体现先后、包含、因果或对比关系。"
        "代码块后附『如何使用这张图学习』和一个闭卷复述任务。"

    ),

    "quiz": (

        "生成『练习题集』Markdown，只允许单选题和判断题，至少 8 题。默认分层为 3 道基础理解、"
        "3 道应用判断、2 道易错辨析；适合考试时再增加 2 道应试题。"
        "每题使用『## 第 N 题（基础/应用/易错/应试）』，明确题型、目标知识点和具体题干。"
        "单选题必须有 A-D 四个可信选项、唯一答案、详细解析、误区诊断，并逐项说明每个错误选项的真实误区；"
        "判断题必须是可判真假的具体命题，给出 T/F、判断依据及错因。答案与详解紧跟每题。"
        "输出前逐题复算：answer 必须存在于 options，解析必须明确支持 answer；数学题最终计算结果必须落在答案选项中。"
        "禁止所有题都考定义，禁止『本节是否重要』『复述核心直觉』等无意义题，禁止弱干扰项，禁止 JSON。"

    ),

    "reading": (

        "生成『拓展阅读』Markdown：推荐 3–5 条材料或阅读方向。每条必须说明为什么推荐、"
        "难度（基础/进阶/拓展）、阅读重点、阅读后必须回答的问题。只有上下文提供或可确认的真实来源才能写具体名称或链接；"
        "无法确认时写『建议搜索关键词』，绝不编造论文、教材、作者、机构或网址。"

    ),

    "media": (

        "生成『多模态讲解』Markdown，优先用流程图、结构图、变量关系图、对比图或步骤图解释知识。"
        "若输出视频脚本，提供 3–6 个分镜表格（镜号|对应知识点|画面内容|旁白|屏幕文字|时长），"
        "每个镜头说明它帮助理解什么，并给出可直接绘制的 Mermaid 教学图。"
        "禁止科技风背景、装饰性插画、标题卡等充当主要内容；画面变化必须对应变量、步骤、因果或对比关系。"

    ),

    "code": (

        "生成『代码案例』Markdown：明确对应知识点、运行环境与版本、实践目标、输入输出。"
        "必须给出从导入到入口都完整可运行的代码，使用清晰变量名和充分中文注释；"
        "随后给出关键代码逐段解释、核心函数说明、运行步骤、至少一个样例输入与精确样例输出、"
        "常见错误及解决方法、扩展任务。算法或机器学习代码还要解释关键变量、数组形状和计算过程。"
        "禁止伪代码、省略号和『此处自行补充』。"
    ...
    "ppt": (
        ...
        "生成『课件提纲』Markdown：8–12 页，每页使用『## 第 N 页：标题』，并包含讲解重点、"
        "控制密度的页面内容、教师讲解提示、互动问题或小检查、是否适合配图及建议图型。"
        "至少有 1 个带完整讲解的例题页和 1 个总结页。"

    ),

    "design": (

        "生成『教学设计』Markdown：必须包含教学目标、学情分析、重点难点、完整教学流程（含时间）、"
        "互动设计、小测设计（含示例题与答案）、评价方式、课后任务，以及每个环节如何调用 AI 课堂。"

    ),

    "project": (

        "生成『实践项目任务书』Markdown：必须包含真实项目背景、实践目标、输入材料、可执行任务步骤、"
        "交付成果、可量化评价标准、分工建议、扩展方向、至少 2 个风险与对应解决方案。"
        "步骤要能让学生直接开工，禁止只写项目名称或空泛里程碑。"
...
def resource_generation_user(
    ...
    *,
    ...
    existing_resource_summaries: list[dict] | None = None,
    ...
    if realtime_summary:
        parts.append(f"当前学习状态：{realtime_summary}")
    ...
    if learning_purpose:
        parts.append(f"学习用途：{learning_purpose}")

    if target_knowledge_points:
        parts.append(f"目标知识点：{'、'.join(target_knowledge_points[:8])}")

    if recommended_stage:
        parts.append(f"适用阶段：{recommended_stage}")

    if expected_outcome:
        parts.append(f"预期学习结果：{expected_outcome}")

    if existing_resource_summaries:
        existing_lines = [
            f"- {row.get('type', '')}｜{row.get('title', '')}｜知识点：{'、'.join(row.get('knowledge_points') or [])}"
            for row in existing_resource_summaries[:10]
            if isinstance(row, dict)
        ]
        if existing_lines:
            parts.append(
                "已有正式资源（必须避免重复；若知识点相同，改用新的例题、任务、误区或应用角度）：\n"
                + "\n".join(existing_lines)
            )
    ...
    if variant_total > 1:
        ...
        parts.append("本类型只生成 1 份：提高单份资源密度，不得减少例题、详解、题量、运行说明或验收任务。")
