...
| 方式 | 命令/操作 | 说明 |
|------|-----------|------|
| Windows 一键 | 双击 `start.bat` 或 `.\scripts\start.ps1` | 等待后端 `/api/health` 就绪后打开浏览器 |
...
| 模式 | user_id | 说明 |
|------|---------|------|
| 快速体验 | `demo` | 登录页「快速体验」；收藏/部分资料可经 `user_preferences` 持久化 |
...
**组件**：`ResourcesContent.tsx`、`ResourceLibraryPanel.tsx`、`ResourceJourneyView.tsx`  
**赛题**：FR-02 多类型资源展示；测试数据要求「自建课程知识库」

#### 应实现功能

| 编号 | 功能 | 优先级 |
|------|------|--------|
| R-01 | 按**学习阶段 + 类型**浏览已生成资源 | P0 |
| R-02 | **课程资料库** Tab：内置库 + 用户上传多文件 | P0 |
| R-03 | 上传经 LLM 分析入库（Chroma） | P0 |
| R-04 | 生成时选择资料库 / 新建库 / 无库全网检索 | P0 |
| R-05 | 关键词搜索 + 类型 Chip 筛选 | P1 |
| R-06 | SSE 生成进度（含 context、web_research） | P0 |
| R-07 | 预览、收藏、测验提交 | P0 |
...
| 步骤 | 操作 | 预期结果 |
|------|------|----------|
| 1 | 打开「课程资料库」Tab | 见「机器学习导论」「Python 程序设计」等内置库，片段数 > 0 |
| 2 | 新建资料库并上传 `.md` | 分析成功，文件数/片段数增加 |
| 3 | 选择内置库 + 主题生成 | 进度含「准备资料上下文」，不含「全网检索整理」 |
| 4 | 选「无资料库·全网检索」生成 | 进度含「全网检索整理」 |
| 5 | 阶段折叠/展开、搜索筛选 | 交互正常 |
| 6 | 预览 doc / quiz | Markdown 与测验面板正常 |
...
| `POST /api/libraries/{id}/upload` | 多文件上传 |
| `GET /api/resources?user_id=` | 已生成资源列表 |
| `POST /api/resources/generate/stream` | `library_id` / `new_library_name` / `topic` |

详细管线见 [07-资源库与生成管线.md](./07-资源库与生成管线.md)。
...
| 页面 | 主要 API | 方法 |
|------|----------|------|
| Login | `/api/auth/send-otp`、`/api/auth/verify-otp`、`/api/auth/demo-token` | POST |
| Chat | `/api/chat/stream`（含 intent/progress/resources/profile）、`/api/health` | POST/GET |
| Tutor | `/api/tutor/stream` | POST |
| Eval | `/api/eval/submit` | POST |
| 全部业务 API | Header `Authorization: Bearer {JWT}` | — |
...
```
[ ] start.bat / start.ps1 可启动且 health 200（DEV_RELOAD=true 时改后端自动 reload）
[ ] 快速体验登录获取 JWT 后全流程
[ ] 对话流式含 progress/resources 事件 + 深度思考开关 + 画像页有数据
[ ] 对话历史刷新后仍在；资源卡片点击跳转资源库预览
[ ] 资源「生成新资源」与「完整套件」SSE 进度 + 下载 .md + 收藏持久化
[ ] GET recommendations 侧栏/对话展示今日推荐
[ ] 预览/完成埋点；题库提交 eval/submit 后路径与评估自动刷新
[ ] 路径 PATCH 标记完成；深色下总进度卡可读
[ ] 设置开启 TTS：有 Key 播后端音频，无 Key 浏览器朗读
[ ] 辅导快捷语走 tutor/stream
[ ] 评估页 study_days / 事件时间线随学习行为变化
[ ] 个人主页编辑同步侧栏
[ ] 深色主题 + 登录页浅色
[ ] 七页侧栏切换无空白
[ ] 退出登录清除 token
...
| v1.1 | 2026-05-22 | 全功能补全：推荐推送、路径 PATCH、扩展资源、TTS、对话历史、generate/stream |
