...
| **资料库** | **用户上传资料库**（PDF/DOCX/PPTX/XLSX/代码等）；**快速入库**（默认跳过逐文件 LLM）；**新建资料库与 AI 生成解耦**；双内置课程 | 上传自己的课件秒级可用，不必被迫走生成流程 |
| **资源生成 UX** | **按类型 0–3 数量滚轮**；**SSE 百分比进度条**；生成阶段全程可见；防 LLM 复述提示词/JSON 脏数据 | 可控批量生成、进度可感知、输出更干净 |
...
- **新建资料库**：仅填写名称 + 上传文件 → **创建并入库**，不触发 AI 资源生成
- **默认快速入库**（`LIBRARY_FAST_INGEST=true`）：解析文件 → 分块 → 向量化，**跳过**逐文件 LLM 分析与库级合成，通常数秒完成
- 资料库详情页有**上传进度条**；简介自动提取为可读文案（兼容历史 JSON 脏数据）
- 需要 AI 深度分析资料库时，可在 `.env` 设 `LIBRARY_FAST_INGEST=false`

#### 3.2 AI 生成学习资源（三选一依据）

1. **依据已有资料库** — RAG 检索 + 可选 LLM 补充
2. **新建资料库**（在生成弹窗内）— 先入库再按所选类型生成（与 3.1 独立入口不同）
3. **无资料库 · 全网检索** — LLM 规划检索主题并整理可信摘要后生成

#### 3.3 资源类型与数量（9 类，每类 0–3 个）

每种类型右侧有 **滚轮计数器**（`InputNumber`），单次生成每类最多 **3** 个；点击类型名可快速 0↔1 切换。预设：**标准套件**（6 类各 1）、**全选 ×1**、**全不选**。
...
同类型生成多份时，标题带「第 N 份」，Prompt 要求角度差异化。

#### 3.4 生成进度与质量

- 生成弹窗内 **百分比进度条** + 阶段文案（上下文 → 各类型 → 质检），上传与生成衔接时进度不中断
- SSE 接口 `POST /api/resources/generate/stream`，请求体支持 `resource_type_counts`，例如：`{"doc": 2, "quiz": 1}`
- **防幻觉增强**：资源正文与资料库简介会检测 LLM 是否复述生成指令或输出原始 JSON，异常时重试或模板回退
- 支持单资源重生成、下载、收藏、清除未收藏项
...
| `LIBRARY_FAST_INGEST` | 资料库上传快速入库（跳过逐文件 LLM 分析） | `true` |
...
| 文档 | 内容 |
|------|------|
| [docs/00-使用指南.md](./docs/00-使用指南.md) | 产品使用流程、模块说明与注意事项（**Web 文档站默认页**） |
...
访问前端 **`/docs`** 可打开内置文档站（静态预渲染，无需登录），默认展示使用指南。
